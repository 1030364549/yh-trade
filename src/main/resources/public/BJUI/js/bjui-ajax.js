...
+function ($) {
    ...
    Bjuiajax.prototype.dialogCallback = function(json) {
        ...
        if (json.divid)
            setTimeout(function() { that.$element.bjuiajax('refreshDiv', json.divid) }, 100)
        if (json.addedid!=null || json.addedid!=""){
        	setTimeout(function() { $("button[data-add='btn-blue-search']").click() }, 100)
        }
    ...
    $(document).on('click.bjui.bjuiajax.data-api', '[data-toggle="resetSearchCon"]', function(e) {
        var $this = $(this), options
        var $form = $this.closest('form')
        ...
        $form.find('input').each(function(i,v) {
            ...
            if(vName!="carte_id" && vName!="showType" && vName!="pageNum" && vName!="totalRecord"
                && vName!="numPerPage"){
                $(v).val("")
            }
        });
        ...
        $form.find('select').each(function(i,v) {
            $(v).find("option:first").prop("selected", 'selected');
        });
        //结束
