                ...
                if (buttons) {
                    for (var i = 0; i < buttons.length; i++) {
                        ...
                        var sIco = (buttons[i].cls && buttons[i].cls == 'green') ? 'check' : 'close'
                        	if(msg!="会话超时，请重新登录。"){
                        	      btnsHtml += FRAG.alertBtnFrag.replace('#btnMsg#', '<i class="fa fa-'+ sIco +'"></i> '+ buttons[i].name).replace('#callback#', sRel).replace('#class#', sCls).replace('#aa#', 'a')
                             }else{
                                btnsHtml += FRAG.alertBtnFrag.replace('#btnMsg#', '<i class="fa fa-'+ sIco +'"></i> '+ buttons[i].name).replace('#callback#', sRel).replace('#class#', sCls).replace('#aa#', 'onclick')
                             }  
                    }
                }
                $newbox = 
