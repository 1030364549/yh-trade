    ...
    $(document).on(BJUI.eventType.initUI, function(e) {
        ...
        $icheck.filter('.checkboxCtrl').on('ifChanged', function(e) {
            var $this     = $(this),
                ...
                $ichecked = $table.closest('.unitBox').find('#bjui-ichecked')
            ...
            if(group == undefined){group=$($this).attr("name")}
            var ids="";
            //循环checkbox
            //获取所有的复选框
            var icks=$table.find('input[name="order_ids"]:checkbox');
            icks.each(function(i,v){
                if($(v).prop("checked")){
                    ids+=($(v).val()+",");
                }
            });
            ...
            if($($ichecked).val()==undefined){
                $ichecked = $('<input type="hidden" id="bjui-ichecked" value="'+ ids +'">')
                $ichecked.appendTo($table.closest('.unitBox'))
            }else{
                $($ichecked).val(ids)
            }
2        })
